...
/* -------- Marketing strip -------- */
.za-strip { border-top:1px solid var(--line); border-bottom:1px solid var(--line); }
.za-strip-grid { display:grid; grid-template-columns:repeat(3,1fr); }
@media(max-width:760px){ .za-strip-grid { grid-template-columns:1fr; } }
.za-strip-cell {
  padding:40px 28px; border-right:1px solid var(--line);
}
.za-strip-cell:last-child { border-right:none; }
@media(max-width:760px){
  .za-strip-cell { border-right:none; border-bottom:1px solid var(--line); padding:32px 4px; }
  .za-strip-cell:last-child { border-bottom:none; }
}
.za-strip-cell h4 {
  font-family:'Fraunces',serif; font-weight:400; font-size:19px;
  margin-bottom:8px; color:var(--text);
}
.za-strip-cell h4 span { color:var(--gold); }
.za-strip-cell p { font-size:14px; color:var(--muted); line-height:1.55; }
...
const PILLARS = [
  { icon:"⊞", title:"Data Analytics",    desc:"Exploration, nettoyage et analyse de vos données pour en sortir ce qui compte.",  tags:["SQL","Python","Pandas"] },
  { icon:"▦", title:"BI & Reporting",     desc:"Dashboards exécutifs lisibles, conçus pour la décision, pas la décoration.",     tags:["Power BI","DAX","Power Query"] },
  { icon:"Σ", title:"Finance & FP&A",     desc:"Budget vs réel, forecasting, variances, design de KPIs qui parlent au CODIR.", tags:["Budgeting","Forecasting","Variances"] },
  { icon:"⇄", title:"Modélisation data",  desc:"Données structurées, fiables et documentées avec dbt, prêtes à être analysées.",tags:["dbt","Modélisation","Tests"] },
];
...
const SERVICES = [
  ...
  {
    slug:"data-engineering-dbt", title:"Structuration de données (dbt)", duration:"2–3 semaines", pricing:"Devis selon scope",
    desc:"Vos données mises au propre, fiables et prêtes à être analysées.",
...
function ContactForm({initial}){
  ...
  const submit=async()=>{
    ...
    setLoading(true);
    try {
      const res = await fetch('https://formspree.io/f/meewvrvq', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          offer: form.offer || '(non spécifié)',
          budget: form.budget || '(non spécifié)',
          message: form.message
        })
      });
      if(res.ok) setSent(true);
      else alert('Erreur lors de l\'envoi. Essaie à nouveau.');
    } catch(err) {
      alert('Erreur réseau. Essaie à nouveau.');
    }
    setLoading(false);
  };
  if(sent) return(
  ...
  return(
    <div>
      ...
      <button className="za-btn" onClick={submit} disabled={loading}>{loading ? "Envoi..." : "Demander un devis"}</button>
    </div>
  );
}
...
function Home({go}){
  return(
    <div className="za-page">
      <header className="za-hero">
        <div className="za-con">
          <div className="za-hero-grid">
            <div>
              <R><div className="za-eyebrow">Data Analyst & FP&A</div></R>
              <R delay=".07s">
                <h1 className="za-h1">Du pipeline à <em>l'insight</em> : vos données financières, prêtes à décider.</h1>
              </R>
              <R delay=".14s">
                <p className="za-hero-sub">J'analyse vos données financières et je les transforme en dashboards clairs et en décisions concrètes. La rigueur d'un analyste finance, les outils data d'aujourd'hui.</p>
      ...
      <section className="za-strip">
        <div className="za-con">
          <R>
            <div className="za-strip-grid">
              <div className="za-strip-cell">
                <h4>Vos chiffres, <span>enfin lisibles</span></h4>
                <p>Fini les fichiers Excel à rallonge que personne n'ouvre. Vos données deviennent des dashboards où l'info utile saute aux yeux en trois secondes.</p>
              </div>
              <div className="za-strip-cell">
                <h4>Un seul interlocuteur, <span>les deux casquettes</span></h4>
                <p>Pas besoin de jongler entre un financier et un technicien. Je comprends vos enjeux métier et je sais manipuler vos données moi-même.</p>
              </div>
              <div className="za-strip-cell">
                <h4>Vous repartez <span>autonome</span></h4>
                <p>Chaque mission est documentée et expliquée. Le but n'est pas de vous rendre dépendant, mais de vous laisser un outil que votre équipe maîtrise.</p>
              </div>
            </div>
          </R>
        </div>
      ...
      <section className="za-sec">
        <div className="za-con">
          ...
          <h2 className="za-h2">Double compétence rare : finance × data</h2>
          <p className="za-lede">La plupart des consultants comprennent soit vos chiffres, soit vos outils — rarement les deux. Je parle la langue de votre DAF autant que celle de vos données, et c'est ce qui fait gagner du temps à tout le monde.</p></R>
          <R delay=".06s">
          ...
          <div className="za-agrid" style={{marginTop:40}}>
            ...
            <R delay=".08s">
              <div className="za-atext">
                <p>Je suis <strong>data analyst spécialisé en FP&A</strong>. Je pars de vos données financières — souvent éparpillées entre Excel, exports et outils — et j'en fais des dashboards clairs et des analyses qui éclairent vos décisions. Pas de jargon, pas de boîte noire : vous comprenez chaque chiffre.</p>
                <p>Mon parcours combine <strong>analyse financière</strong> (budgeting, forecasting, variance analysis) et <strong>maîtrise data</strong> (SQL, Python, Power BI, et dbt pour structurer les données). Deux compétences, une seule personne en face de vous.</p>
        ...
        <div className="za-con">
          <div className="za-sitemap">
            <div className="za-sitemap-brand">
              <button className="za-logo" onClick={()=>go("home")}>Zafyr<span>·</span>Analytics</button>
              <p style={{marginTop:10}}>Data Analyst & FP&A — j'analyse vos données financières et je les transforme en dashboards clairs et en décisions concrètes.</p>
            </div>
            <div>
              <h5>Navigation</h5>
              <ul>
